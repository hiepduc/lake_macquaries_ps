...
plt.plot(np.arange(len(diurnal)), diurnal)

plt.xlabel("Hour")
plt.ylabel("PM2.5 (ug m-3)")
plt.title("Domain Mean Diurnal PM2.5 all")

plt.grid()

plt.tight_layout()
plt.savefig("PM25_diurnal_all.png", dpi=300)
plt.show()
...
domain_mean = pm25.mean(dim=["ROW","COL"]).values

nhr = len(domain_mean)

hours = np.arange(nhr) % 24

diurnal = np.array([
    domain_mean[hours == h].mean()
    for h in range(24)
])

plt.figure(figsize=(8,4))

plt.plot(np.arange(24), diurnal, marker="o")

plt.xlabel("Hour")
plt.ylabel("PM2.5 (ug m-3)")
plt.title("Mean Diurnal PM2.5")
